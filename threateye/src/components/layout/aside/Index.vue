  ...
  <div id="aside"
    ...
    <div class="aside-breadcrumb">
      <template v-if="!isCollapse">
        <el-breadcrumb class="all-breadcrumb"
                       separator-class="el-icon-arrow-right">
          <el-breadcrumb-item :to={path:crumb.path}>{{crumb.name}}</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="aside-crumb"
             align="right"
             @click.stop="toggleCollapse();">
          <img class="crumb-image"
               :src="crumbLeftSrc">
        </div>
      </template>
      <template v-else>
        <div class="aside-crumb-right"
             align="right"
             @click.stop="toggleCollapse();">
          <img class="crumb-image"
               :src="crumbRightSrc">
    ...
    <transition name="fade">
      <el-menu :default-active="$route.meta.auth"
               :collapse="isCollapse"
               v-cloak>
        <aside-item v-for="item in asideList"
                    :key="item.meta.auth"
                    :item="item"
