...
export default {
  ...
  actions: {
    ...
    async getScreenBase ({ commit, dispatch }, context) {
      let resp = await axios('/yiiapi/demonstrationoem/get-base-config', { params: context });
    ...
    async getScreenAside ({ commit, dispatch }, context) {
      let resp = await axios('/yiiapi/demonstrationoem/get-both-side', { params: context });
      let { status, data } = resp.data;
      //console.log(data)
      if (status == 0) {
    ...
    async setScreenAside ({ commit, dispatch }, context) {
      ...
      let resp = await axios.put('/yiiapi/demonstrationoem/set-both-side', { param: asideLists });
      let { status, data } = resp.data;
      if (status == 0) {
        return true;
      }
    ...
    async getScreenTop ({ commit, dispatch }, context) {
      let resp = await axios('/yiiapi/demonstrationoem/get-top-config', { params: context });
      let { status, data } = resp.data;
      data.forEach(item => {
        if (!item.num) {
          item.num = [0, 0, 0, 0, 0];
    ...
    async setScreenTop ({ commit, dispatch }, context) {
      ...
      let resp = await axios.put('/yiiapi/demonstrationoem/set-top-config', { param: topLists });
      let { status, data } = resp.data;
      if (status == 0) {
        return true;
      }
