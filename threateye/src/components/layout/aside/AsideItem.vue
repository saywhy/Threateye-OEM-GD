    ...
    <template v-if="!item.children">
      <router-link class="item"
        ...
        <el-menu-item :index="String(item.meta.auth)"
                      v-show="item.meta.title != '沙箱' ? true : sandbox">
          <i v-if="item.meta.deep == 1"
             class="e-aside-icon"
             :class="item.meta.icon"></i>
          <span slot="title"
                class="title">{{item.meta.title}}</span>
        </el-menu-item>
      </router-link>
    </template>

    <el-submenu v-else
                :index="item.meta.auth">
      <template slot="title">
        <i v-if="item.meta.deep == 1"
           class="e-aside-icon"
           :class="item.meta.icon"></i>
      ...
      <aside-item v-for="child in item.children"
                  :key="child.path"
                  :item="child"></aside-item>
    </el-submenu>
